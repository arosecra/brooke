...
export class SettingsComponent {
  ...
  sampleText: WritableSignal<string> = signal<string>(
  ...
  cacheDirectory = asyncComputed(async () => {
    const cacheDir =
      this.app.settings.value()?.cacheDirectory;
    if (!cacheDir)
      return {
  ...
  async requestCollectionPermission(
    permissable: Collection,
  ) {
    this.busy.set(true);
    this.busyMessage.set(['Requesting Permission']);
    permissable.hasRWPermission = await WebFS.getPermission(
      permissable.handle,
      'readwrite',
    );
    permissable.hasRPermission = await WebFS.getPermission(
      permissable.handle,
      'read',
    );
    this.app.storedLibrary.reload();
    this.busy.set(false);
  }

  async requestCacheDirPermission(
  ...
  async setLocalCacheDirectory() {
    ...
    resourceStatusToPromise(this.app.storedLibrary).then(
      () => {
        this.busy.set(false);
      },
    );
  ...
  async addCollectionFromHandle(
    ...
    children.forEach((child) => {
      ...
      else if (child.name === 'collection.yaml')
        collectionHandle = child;
      else if (child.name === 'categories.yaml')
        categoriesHandle = child;
    });
    ...
    collection.displayName = collection.name.replaceAll(
      '_',
      ' ',
    );
    ...
    category.forEach((cat) => {
      cat.items.forEach((itemRef) => {
        itemRef.displayName = itemRef.name
          .replaceAll(cat.name, '')
          .replaceAll('_', ' ');
    ...
    for (let i = 0; i < childDirs.length; i++) {
      ...
      await WebFS.onLeafDirs(
        ...
        async (
          ...
          leaf: FileSystemDirectoryHandle,
        ) => {
          this.busyMessage.set([
            'Collection and Categories added.',
          ...
          if (parentDirInterogation?.name !== parent.name)
            parentDirInterogation = await this.possibleItem(
              collection,
              parent,
            );

          const intero = await this.possibleItem(
            collection,
            leaf,
          );
          ...
          if (intero.item) {
            if (
              parentDirInterogation.thumbnail &&
              index === 0
            ) {
              this.busyMessage.set([
                'Collection and Categories added.',
              ...
              await this.appDB.addThumbnail({
                ...
                collectionName: collection.name,
                categoryName:
                  itemToCategory[parent.name]?.name ??
                  'unassigned',
                thumbnail:
                  await parentDirInterogation.thumbnail.getFile(),
              });
            }
            ...
            await this.appDB.addItem({
              ...
              dirHandle: leaf,
              series: false,
              bookDetails: intero.cbtDetails
                ? await WebFS.readYaml(intero.cbtDetails)
                : undefined,
              childItems: [],
            });
  ...
  async possibleItem(
    ...
    files.forEach((file) => {
      ...
      if (
        file.kind === 'file' &&
        file.name.startsWith('cbtDetails')
      )
        cbtDetails = file;
      if (
        file.kind === 'file' &&
        file.name.endsWith('.ocr.gz')
      )
        ocr = file;
      if (
        file.kind === 'file' &&
        file.name.endsWith('.tmb.gz')
      )
        thumbs = file;
      if (
        file.kind === 'file' &&
        file.name.endsWith(collection.itemExtension)
      )
        item = file;
    });
  ...
  saveVoice() {
    ...
    this.appDB.updateSettings(newSettings).then(() => {
      this.app.storedLibrary.reload();
  ...
  changeVoice($event: string) {
    this.app.settings.value()!.voice = $event;
  }
}
