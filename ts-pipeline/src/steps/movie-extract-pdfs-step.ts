...
export class BookExtractPDFsStep implements JobStep {
	execute(job: JobFolder): void {
		...
		for(let i = 0; i < workFiles.length; i++) {
			...
			if(workFile.endsWith("pdf") && !workFile.includes('box')) {
				pdfCount++;
			}
		}
...
//				jss.startAndPrint();
			...
			//java would crop and rotate if height > width
			//  TODO - just find the ones that need to be cropped & crop em manually
		}
